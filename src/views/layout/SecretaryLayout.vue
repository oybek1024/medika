<template>
  <div class="min-h-screen">
    ...
    <div class="h-t_m_menu sm:h-t_menu grid grid-cols-12 gap-x-2 items-center justify-center w-full text-tgreen">
      <p :class="[secretaryId ? 'col-start-0 col-end-3': 'col-start-5 col-end-9', 'sm:col-start-6 sm:col-end-8 font-bold text-center text-xl sm:text-3xl']">{{ title }}</p>
      <p v-if="secretaryId"
        @click="logout"
...
export default {
  ...
  computed: {
    ...mapState({
      secretaryId: (state) => state.secretary.secretaryId
    }),
  },
  methods: {
    ...mapActions({
      initSecretary: (actions) => actions.secretary.init
    }),
    logout() {
      this.$myAuthAxios({
        url: '/secretary/logout',
        method: 'POST'
      })
      .then((response) => {
      ...
      .catch(err => {
        ...
        this.$router.push({ name: 'secretary-login' });
      })
